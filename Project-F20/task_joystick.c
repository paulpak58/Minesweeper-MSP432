...
void Task_Joystick_Bottom_Half(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        prev_accel_msg.cmd = accel_msg.cmd;

        if(ACCEL_X_DIR < TILT_NEG)
        {
            accel_msg.cmd = MOUSE_CMD_LEFT;
        }
        else if(ACCEL_X_DIR > TILT_POS)
        {
            accel_msg.cmd = MOUSE_CMD_RIGHT;
        }
        else if(ACCEL_Y_DIR < TILT_NEG)
        {
            accel_msg.cmd = MOUSE_CMD_DOWN;
        }
        else if(ACCEL_Y_DIR > TILT_POS)
        {
            accel_msg.cmd = MOUSE_CMD_UP;
        }
        else {
            accel_msg.cmd = MOUSE_CMD_CENTER;
        }
        ...
        if(!(prev_accel_msg.cmd == MOUSE_CMD_CENTER && accel_msg.cmd == MOUSE_CMD_CENTER)) {
            xQueueSendToBack(Queue_Mouse, &accel_msg, portMAX_DELAY);
        }
    }
}
...
void ADC14_IRQHandler(void)
{
    ...
    ACCEL_X_DIR = ADC14->MEM[2];        //Read X and clear interrupt
    ACCEL_Y_DIR = ADC14->MEM[3];        //Read Y and clear interrupt
