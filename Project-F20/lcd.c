...
void HAL_LCD_SpiInit(void)
{
    ...
    EUSCI_B0->CTLW0 =       EUSCI_B_CTLW0_CKPH |        // Phase of 1
                            ...
                            EUSCI_B_CTLW0_MODE0 |       // 3 pin SPI Mode
    ...
    LCD_DC_PORT->OUT |= LCD_DC_PIN;     // data in form of image data (instead of control commands)
}
...
void Crystalfontz128x128_SetDrawFrame(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    // used to set the Column start and column ends.
    HAL_LCD_writeCommand(CM_CASET);
    HAL_LCD_writeData(x0 >> 8);     // top half
    HAL_LCD_writeData(x0);          // bottom half
    ...
    HAL_LCD_writeData(y0 >> 8);     // top half
    HAL_LCD_writeData(y0);          // bottom half
